...
export function App() {
  ...
  const [selectedListId, setSelectedListId] = useState<number | "new" | null>(
    null
  );
  ...
  async function handleSave() {
    ...
    try {
      const listName =
        lists.find((l) => l.id === selectedListId)?.name ?? "your list";
  ...
  if (phase === "loading" || scrapeState.status === "loading") {
    return (
      <div className="p-3">
        <StatusBanner type="loading" message="Reading recipe…" />
      </div>
    );
  }
  ...
  const browseButton = (
    <button
      ...
      className="w-full py-1.5 rounded border border-gray-200 text-xs text-black hover:bg-gray-50 transition-colors"
    >
    ...
    return (
      <div className="p-3 flex flex-col gap-3">
        <p className="text-sm text-gray-600">
          Navigate to an NYT Cooking recipe page to save it.
        </p>
        {browseButton}
      </div>
    );
